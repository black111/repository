...
//			System.out.println(html);
		...
		w_hotel_info.writer2file("C:/Users/LivelyRong/Desktop/xc/hotellib.txt", "酒店代码："+hotel_code
				+" 酒店名称："+hotel_name
				+" 酒店电话："+phone
...
//		    //此区域的酒店源码
		...
		//获得这个区域酒店的页数
		//*[@id="pageContainer"]
		Elements pages=doc.select("[method=\"page\"]");		
		Maxpage=Integer.parseInt(pages.last().attr("data-index").toString());
		...
		for(int i=1;i<=7;i++){
			...
			//获得此页酒店的超链接
			Elements hrefs_hotel=doc1.select("[class=h_item]");			
			System.out.println(hrefs_hotel.size());
			//一页酒店的所有超链接
			String href="";
			for(int j=0;j<hrefs_hotel.size();j++){    
				//获得这一页所有酒店的超链接。。
				String hotel_code=driver.findElement(By.xpath("//*[@class=\"h_list\"]/div["+(j+1)+"]")).getAttribute("data-hotelid");
				if(hotel_code!=null){
					count++;
					list_hotel_href.add("http://hotel.elong.com/#review"+hotel_code+"/");
					if(j!=hrefs_hotel.size()-1){
						href+="http://hotel.elong.com/"+hotel_code+"/#review"+'\n';					
			...
			w_href_toFile.writer2file("C:/Users/LivelyRong/Desktop/Url/yl.txt", href);
			//获取下一页的链接
			try{
				driver.findElement(By.xpath("//*[@class=\"page_next\"]")).click();
